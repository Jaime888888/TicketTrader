...
(function () {
  ...
  const formatPriceLabel = (min, max) => {
    ...
    return `${hasMin ? minVal : '?'} - ${hasMax ? maxVal : '?'}`;
  };
  ...
  const renderDetail = (fav, detail) => {
    ...
    const priceLabel = formatPriceLabel(min, max);
  ...
  function render() {
    ...
    for (const fav of favs) {
      ...
      price.textContent = `Price range: ${formatPriceLabel(fav.minPriceUsd, fav.maxPriceUsd)}`;
